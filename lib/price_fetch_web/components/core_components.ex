defmodule PriceFetchWeb.CoreComponents do
  ...
  import PriceFetchWeb.Gettext

  def button(assigns) do
    ~H"""
    <button
      type={@type}
      class="text-white absolute end-2.5 bottom-2.5 bg-slate-600 hover:bg-slate-500 focus:ring-4 focus:outline-none focus:ring-slate-300 font-medium rounded-lg text-sm px-4 py-2 "
    >
      <%= @text %>
    </button>
    """
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(PriceFetchWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(PriceFetchWeb.Gettext, "errors", msg, opts)
    end
  end
